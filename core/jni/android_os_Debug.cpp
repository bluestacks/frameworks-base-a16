...
static jboolean android_os_Debug_isVmapStack(JNIEnv *env, jobject clazz)
{
    ...
    if (cfg_state == CONFIG_UNKNOWN) {
        ...
        // BS-A16: /proc/config.gz does not exist on BlueStacks kernel.
        // Treat missing kernel config as CONFIG_UNSET instead of aborting.
        if (result != OK) {
            ALOGW("Kernel configs could not be fetched (no /proc/config.gz). "
                  "Assuming CONFIG_VMAP_STACK=n. b/151092221");
