...
public class SecureLockDeviceService extends SecureLockDeviceServiceInternal {
    ...
    private boolean enableSecureLockDeviceAfterBoot() {
        ...
        // TODO (b/406265894): Notify listener of secure lock device update
        if (DEBUG) {
    ...
    public int isSecureLockDeviceAvailable(UserHandle user) {
        ...
        if (mBiometricManager == null) {
            ...
            return ERROR_UNSUPPORTED;
        } else if (!mBiometricManager.hasEnrolledBiometrics(user.getIdentifier())) {
            if (DEBUG) {
        ...
        } else if (mBiometricManager.canAuthenticate(user.getIdentifier(),
